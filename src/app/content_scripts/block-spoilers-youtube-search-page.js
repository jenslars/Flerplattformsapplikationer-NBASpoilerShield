(async function() {
    ...
    console.log('Vi är youtube search page')
    ...
    function hideSpoilers() {
        document.querySelectorAll('ytd-video-renderer, ytd-reel-item-renderer').forEach(renderer => {
            const isVideo = renderer.matches('ytd-video-renderer');
            const isReel = renderer.matches('ytd-reel-item-renderer');

            if (isVideo) {
                checkAndApplySpoilers(renderer, '#video-title', 'ytd-video-renderer-overlay', 'ytd-video-renderer-spoiler-content');
            } else if (isReel) {
